...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = nn.Parameter(torch.randn(1, self.hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_to_hidden_bias)
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(torch.randn(self.hidden_size, self.n_classes))
        self.hidden_to_logits_bias = nn.Parameter(torch.randn(1, self.n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        # for b in range()
        x = torch.index_select(self.embeddings.data, 0, torch.flatten(w))
    ...
    def forward(self, w):
        ...
        z1 = torch.add(torch.matmul(x, self.embed_to_hidden_weight), self.embed_to_hidden_bias)
