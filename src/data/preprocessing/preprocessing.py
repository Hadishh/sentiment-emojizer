...
#h
def frequent_emoji_removal(tokenized_tweets, removal_rate = 0.5):
    emojis_count = count_emojis(tokenized_tweet)
    refined_tweets = []
...
def remove_undefined_emojis(tokenized_tweet):
    tweet = []
    info = ClassesInformation(csv_url)
...
def identify_label(tokenized_tweet):
    labels = []
    for token in tokenied_tweet:
        if(token in emoji.UNICODE_EMOJI_ENGLISH):
            labels.append(classes_info.get_emoji_class(token))
    for 